...
export default function($log, $q, $WPBMCHSConfig, CacheFactory, $cordovaToast, $filter, $window) {
    'ngInject';
    ...
    function getCache() {
        ...
        hash = md5($WPBMCHSConfig.api.baseUrl);
        if (CacheFactory.get('bookmark-' + hash)) {
            return CacheFactory.get('bookmark-' + hash);
        }
        cacheOptions = angular.extend(_.get($WPBMCHSConfig, 'bookmark.cache') || {}, {
